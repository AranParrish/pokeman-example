...
class TestAttributes:
    ...
    @pytest.mark.it(
        "Pokemon instances in Battle are same as those passed to constructor"
    )
    ...
    def test_battle_initiliases__current_attacker__to_first_pokemon(
        self, pikachu, flareon
    ):
        test_pokemon_1 = NormalPokemon(*pikachu)
        test_pokemon_2 = FirePokemon(*flareon)
        test_battle = Battle(test_pokemon_1, test_pokemon_2)
...
class TestTakeTurn:
    ...
    @pytest.mark.it(
        "Pokemon 1 deals different damage to pokemon 2 with different combination of pokemon"
    )
    ...
    def test_battle__current_attacker__attribute_is_updated_after__take_turn__called(
    ...
    def test_battle__current_turn__attribute_switches_back_to_0_after__take_turn__called_twice(
        self, leafeon, vaporeon
    ):
        test_pokemon_1 = GrassPokemon(*leafeon)
        test_pokemon_2 = WaterPokemon(*vaporeon)
    ...
    @pytest.mark.it(
        "Battle raises exception when attacking pokemon has already fainted"
    )
...
class TestGetWinner:
    ...
    @pytest.mark.it(
        "get_winner returns None if neither pokemon has fainted after turn taken"
    )
